...
export default class Shop extends Component {
  constructor(props) {
    ...
    this.removeInShopping = this.removeInShopping.bind(this);
  }

  addToShop(id) {
    let findProduct = this.state.products.find((product) => product.id === id);
    this.setState({ shoppingCart: [...this.state.shoppingCart, findProduct] });
  }

  removeInShopping(id) {
    let newShoppingCart = this.state.shoppingCart.filter(
          ...
          <div className="cart-items">
            {this.state.shoppingCart.map((product) => (
              <CartProduct
                key={product.id}
                {...product}
                removeInShopping={this.removeInShopping}
              />
            ))}
          </div>
          <button
            ...
            type="button"
            onClick={() => this.allRemoveShoppingCart()}
          >
            Empty Cart
          </button>
        </section>
